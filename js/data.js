...
window.propertiesReady = (async function () {
  if (!window.UniNestApi) return properties;
  try {
    var apiListings = await window.UniNestApi.getPublishedListings();
    if (apiListings && apiListings.length) {
      properties = apiListings;
    }
  } catch (e) {
    // Keep the local mock catalog if the API is offline.
  }
  return properties;
})();
